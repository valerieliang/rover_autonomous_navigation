"""
main.py  —  Autonomous Navigation Pipeline: Unified Entry Point
================================================================
Fuses Visual Odometry, Scene Understanding, and YOLO Object Detection
into a single real-time loop driven by a Stereolabs ZED stereo camera.

  • One sl.Camera handle, one zed.grab() per frame.
  • VO + scene logic lives in slam/ — no camera calls, independently testable.
  • YOLO inference lives in object_detection.py — no camera, no SLAM.

Usage
-----
  python main.py                              # live camera, all features
  python main.py --svo path/to/file.svo       # SVO replay
  python main.py --model models/best_bw.pt    # custom weights
  python main.py --no-display --no-plot       # headless / Jetson
...
try:
    ...
    sys.exit(
        f"[ERROR] Could not import SLAM helpers: {e}\n"
        "Make sure slam/zed_vo_core.py and slam/zed_scene_core.py exist."
    )
...
try:
    ...
    sys.exit(
        f"[ERROR] Could not import object_detection: {e}\n"
        "Make sure object_detection.py is in the project root."
    )
...
def _build_init_params(args: argparse.Namespace) -> sl.InitParameters:
    ...
    p.camera_resolution      = res_map.get(args.resolution.upper(),
                                           sl.RESOLUTION.HD720)
...
def _draw_vo_hud(frame: np.ndarray, nav: NavState) -> None:
    """Burn VO figures into the top-left corner of frame (in-place)."""
    overlay = frame.copy()
    cv2.rectangle(overlay, (0, 0), (420, 185), (0, 0, 0), -1)
    ...
    put(f"Speed: {nav.speed:.3f} m/s     Frame: {nav.frame_idx}", 2, 0.58)
    put(f"CumDist: {nav.cumulative_distance:.3f} m   "
        f"NetDist: {nav.net_displacement:.3f} m", 3)
...
def run(args: argparse.Namespace) -> None:
    ...
    det_cfg.MODEL_PATH  = args.model
    det_cfg.CONF_THRESH = args.conf
    det_cfg.IOU_THRESH  = args.iou
    ...
    if not args.no_detection:
        try:
            ...
            print(f"[WARN] {e}")
            print("[WARN] Running without object detection.")
            detector = None
    ...
    # ── Console header ────────────────────────────────────────────────────────
    det_col = "  Detections" if detector else ""
    print(f"{'Frame':>6}  {'X':>8}  {'Y':>8}  {'Z':>8}  "
          f"{'Spd m/s':>8}  {'Yaw°':>7}  "
          f"{'LWall':>7}  {'RWall':>7}  {'Hallway':>8}  {'LOS':>7}"
          f"{det_col}")
    print("-" * (105 + (14 if detector else 0)))
    ...
    try:
        while True:
            ...
            ts_s = zed.get_timestamp(
                sl.TIME_REFERENCE.IMAGE).get_nanoseconds() * 1e-9
            dt   = (ts_s - prev_time_s
                    if prev_time_s is not None else 1.0 / max(fps_cam, 1))
            dt   = max(dt, 1e-4)

            # ── Retrieve image + depth (shared by all pipelines) ──────────────
            ...
            if frame_idx % args.verbose_interval == 0:
                ...
                if det_result is not None:
                    det_str = f"  dets={det_result.count}"
                print(
                    f"{frame_idx:>6}  "
                    ...
                    f"{los_obj.dist_m:>7.2f}"
                    f"{det_str}"
                )
                if det_result and det_result.any_detected:
                    print_detections(det_result)
                if imu_available and not args.no_imu:
                    a, g = nav.linear_acceleration, nav.angular_velocity
            ...
            if not args.no_display:
                # 1. Scene overlay (wall highlights, LOS rect, cluster boxes,
                #    scene HUD panel below the image)
                vis = draw_scene_overlay(
                    frame_bgr, depth_clean, scene, clusters, scene_cfg)

                # 2. VO HUD burned into the camera portion only
                ...
                combined = cv2.resize(
                    np.hstack([vis, depth_col]), None, fx=0.75, fy=0.75)

                cv2.imshow("ZED Navigation", combined)
            ...
            if (plotter is not None
                ...
                plotter.update(
                    traj_arr, frame_idx,
                    tracker.net_displacements[-1]
                    if tracker.net_displacements else 0.0,
    ...
    print(f"\n{'='*64}")
    print("NAVIGATION PIPELINE — SESSION SUMMARY")
    print(f"{'='*64}")
...
def _build_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(
        description="Autonomous Navigation Pipeline — ZED VO + Scene + YOLO Detection",
    ...
    p.add_argument("--no-detection", action="store_true",
                   help="Disable YOLO object detection (SLAM + scene only)")
